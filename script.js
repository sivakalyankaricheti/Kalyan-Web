...
Object.assign(UI_TRANSLATIONS.en,{locationLoading:'Loading local time and weather…',locating:'Finding your current location…',yourLocation:'Your location',locationUnavailable:'Weather unavailable',locationPermissionDenied:'Location access was not granted.'});
Object.assign(UI_TRANSLATIONS.fr,{locationLoading:'Chargement de l’heure locale et de la météo…',locating:'Recherche de votre position actuelle…',yourLocation:'Votre position',locationUnavailable:'Météo indisponible',locationPermissionDenied:'L’accès à la position n’a pas été accordé.'});
...
Object.assign(UI_TRANSLATIONS.en,{technicalSkills:'Technical Skills',softSkills:'Soft Skills'});
Object.assign(UI_TRANSLATIONS.fr,{technicalSkills:'Compétences techniques',softSkills:'Compétences humaines'});
...
const softSkillNames=new Set(['adaptability','stakeholder collaboration','curiosity & continuous learning','problem solving','professional communication']);
const skillCategory=item=>item.category==='soft'||(!item.category&&softSkillNames.has((item.name||item.value||'').toLowerCase()))?'soft':'technical';
const weatherIcons={0:'☀',1:'🌤',2:'⛅',3:'☁',45:'🌫',48:'🌫',51:'🌦',53:'🌦',55:'🌦',56:'🌧',57:'🌧',61:'🌧',63:'🌧',65:'🌧',66:'🌧',67:'🌧',71:'🌨',73:'🌨',75:'🌨',77:'🌨',80:'🌦',81:'🌦',82:'🌧',85:'🌨',86:'🌨',95:'⛈',96:'⛈',99:'⛈'};
let portfolioLocationState=null,visitorLocationState=null,locationClockTimer=null,locationRefreshTimer=null,locationAccessDenied=false,visitorLocationRequestPending=false;
const locationStatus=document.getElementById('portfolio-location-status'),visitorLocationStatus=document.getElementById('visitor-location-status');
const formatLocationTime=timezone=>{try{return new Intl.DateTimeFormat(currentLanguage==='fr'?'fr-FR':'en-GB',{hour:'2-digit',minute:'2-digit',timeZone:timezone}).format(new Date())}catch(error){return ''}};
const locationReading=(label,state)=>`${label} · ${formatLocationTime(state.timezone)} · ${weatherIcons[state.weatherCode]||'☁'} ${Math.round(state.temperature)}°C`;
function renderLocationPanel(){if(portfolioLocationState)locationStatus.textContent=locationReading(`📍 ${portfolioLocationState.name}`,portfolioLocationState);if(visitorLocationState){visitorLocationStatus.hidden=false;visitorLocationStatus.textContent=locationReading(`⌖ ${t('yourLocation')}`,visitorLocationState)}}
async function fetchCurrentWeather(latitude,longitude){const response=await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${encodeURIComponent(latitude)}&longitude=${encodeURIComponent(longitude)}&current=temperature_2m,weather_code&timezone=auto`);if(!response.ok)throw new Error('Weather unavailable');const weather=await response.json(),current=weather.current;if(!current||typeof current.temperature_2m!=='number')throw new Error('Weather unavailable');return {temperature:current.temperature_2m,weatherCode:current.weather_code,timezone:weather.timezone||'UTC'}}
async function loadPortfolioLocation(location,force=false){if(portfolioLocationState?.source===location&&!force){renderLocationPanel();return}locationStatus.textContent=t('locationLoading');try{const response=await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(location)}&count=1&language=en&format=json`);if(!response.ok)throw new Error('Location unavailable');const place=(await response.json()).results?.[0];if(!place)throw new Error('Location unavailable');const weather=await fetchCurrentWeather(place.latitude,place.longitude);portfolioLocationState={...weather,name:[place.name,place.country].filter(Boolean).join(', '),source:location};renderLocationPanel();clearInterval(locationClockTimer);locationClockTimer=window.setInterval(renderLocationPanel,60000)}catch(error){locationStatus.textContent=`📍 ${location} · ${t('locationUnavailable')}`}}
function requestCurrentLocation({silent=false}={}){if(!navigator.geolocation){visitorLocationStatus.hidden=false;visitorLocationStatus.textContent=t('locationPermissionDenied');return}if(locationAccessDenied||visitorLocationRequestPending)return;visitorLocationRequestPending=true;if(!silent||!visitorLocationState){visitorLocationStatus.hidden=false;visitorLocationStatus.textContent=t('locating')}navigator.geolocation.getCurrentPosition(async position=>{try{visitorLocationState=await fetchCurrentWeather(position.coords.latitude,position.coords.longitude);renderLocationPanel()}catch(error){if(!visitorLocationState)visitorLocationStatus.textContent=t('locationUnavailable')}finally{visitorLocationRequestPending=false}},error=>{if(error.code===error.PERMISSION_DENIED){locationAccessDenied=true;visitorLocationStatus.hidden=false;visitorLocationStatus.textContent=t('locationPermissionDenied')}else if(!visitorLocationState){visitorLocationStatus.hidden=false;visitorLocationStatus.textContent=t('locationUnavailable')}visitorLocationRequestPending=false},{enableHighAccuracy:false,timeout:10000,maximumAge:240000})}
async function loadPortfolio(){
  try{
    ...
    loadPortfolioLocation(data.location);
    requestCurrentLocation();
    clearInterval(locationRefreshTimer);
    locationRefreshTimer=window.setInterval(()=>{loadPortfolioLocation(data.location,true);requestCurrentLocation({silent:true})},300000);
    ...
    const skillDetail=document.getElementById('skill-detail'),skillTitle=document.getElementById('skill-detail-title'),skillDescription=document.getElementById('skill-detail-description');
    const skillGroups={technical:{cloud:document.getElementById('technical-skill-cloud'),group:document.getElementById('technical-skill-group')},soft:{cloud:document.getElementById('soft-skill-cloud'),group:document.getElementById('soft-skill-group')}};
    const skills=(data.skills||[]).map(value=>typeof value==='string'?{name:value,description:''}:value),groupedSkills={technical:[],soft:[]};skills.forEach(item=>groupedSkills[skillCategory(item)].push(item));
    Object.entries(skillGroups).forEach(([category,entry])=>{const groupSkills=groupedSkills[category];entry.group.hidden=!groupSkills.length;entry.cloud.replaceChildren(...groupSkills.map(item=>{const skill=document.createElement('button');skill.type='button';skill.className='skill-chip';skill.textContent=item.name||item.value||'';skill.addEventListener('click',()=>{document.querySelectorAll('.skill-chip').forEach(node=>{node.classList.remove('active');node.setAttribute('aria-pressed','false')});skill.classList.add('active');skill.setAttribute('aria-pressed','true');skillTitle.textContent=item.name||item.value||'';skillDescription.textContent=item.description||t('skillFallback');entry.group.append(skillDetail);skillDetail.hidden=false;skillDetail.scrollIntoView({behavior:'smooth',block:'nearest'})});skill.setAttribute('aria-pressed','false');return skill}))});
    ...
    const galleryItems=data.gallery||[];document.getElementById('gallery-empty').hidden=galleryItems.length>0;document.getElementById('gallery-grid').replaceChildren(...galleryItems.map(item=>{const card=document.createElement('article');card.className='gallery-card reveal';const isVideo=item.type==='video'||/\.(mp4|webm|mov)(?:[?#]|$)/i.test(item.image||'');const media=document.createElement(isVideo?'video':'img');media.src=item.image;if(isVideo){media.controls=true;media.preload='metadata';media.playsInline=true;media.setAttribute('aria-label',item.title||'Gallery video')}else{media.alt=item.title||'Gallery image';media.loading='lazy'}const hasDetails=item.title||item.description||item.url;if(hasDetails){const body=document.createElement('div');if(item.title){const title=document.createElement('h3');title.textContent=item.title;body.append(title)}if(item.description){const description=document.createElement('p');description.textContent=item.description;body.append(description)}if(item.url){const link=document.createElement('a');link.href=item.url;link.target='_blank';link.rel='noopener noreferrer';link.textContent=t('openLink');body.append(link)}card.append(media,body)}else card.append(media);return card}));
