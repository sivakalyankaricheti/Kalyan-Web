...
Object.assign(UI_TRANSLATIONS.en,{locationLoading:'Loading local time and weather…',useCurrentLocation:'Use my location',locating:'Finding your current location…',yourLocation:'Your location',locationUnavailable:'Weather unavailable',locationPermissionDenied:'Location access was not granted.'});
Object.assign(UI_TRANSLATIONS.fr,{locationLoading:'Chargement de l’heure locale et de la météo…',useCurrentLocation:'Utiliser ma position',locating:'Recherche de votre position actuelle…',yourLocation:'Votre position',locationUnavailable:'Météo indisponible',locationPermissionDenied:'L’accès à la position n’a pas été accordé.'});
...
let portfolioLocationState=null,visitorLocationState=null,locationClockTimer=null;
const locationStatus=document.getElementById('portfolio-location-status'),visitorLocationStatus=document.getElementById('visitor-location-status'),useCurrentLocation=document.getElementById('use-current-location');
...
function renderLocationPanel(){useCurrentLocation.textContent=t('useCurrentLocation');if(portfolioLocationState)locationStatus.textContent=locationReading(`📍 ${portfolioLocationState.name}`,portfolioLocationState);if(visitorLocationState){visitorLocationStatus.hidden=false;visitorLocationStatus.textContent=locationReading(`⌖ ${t('yourLocation')}`,visitorLocationState)}}
...
async function loadPortfolioLocation(location){if(portfolioLocationState?.source===location){renderLocationPanel();return}locationStatus.textContent=t('locationLoading');try{const response=await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(location)}&count=1&language=en&format=json`);if(!response.ok)throw new Error('Location unavailable');const place=(await response.json()).results?.[0];if(!place)throw new Error('Location unavailable');const weather=await fetchCurrentWeather(place.latitude,place.longitude);portfolioLocationState={...weather,name:[place.name,place.country].filter(Boolean).join(', '),source:location};renderLocationPanel();clearInterval(locationClockTimer);locationClockTimer=window.setInterval(renderLocationPanel,60000)}catch(error){locationStatus.textContent=`📍 ${location} · ${t('locationUnavailable')}`}}
function requestCurrentLocation(){if(!navigator.geolocation){visitorLocationStatus.hidden=false;visitorLocationStatus.textContent=t('locationPermissionDenied');return}useCurrentLocation.disabled=true;visitorLocationStatus.hidden=false;visitorLocationStatus.textContent=t('locating');navigator.geolocation.getCurrentPosition(async position=>{try{visitorLocationState=await fetchCurrentWeather(position.coords.latitude,position.coords.longitude);renderLocationPanel()}catch(error){visitorLocationStatus.textContent=t('locationUnavailable')}finally{useCurrentLocation.disabled=false}},()=>{visitorLocationStatus.textContent=t('locationPermissionDenied');useCurrentLocation.disabled=false},{enableHighAccuracy:false,timeout:10000,maximumAge:300000})}
