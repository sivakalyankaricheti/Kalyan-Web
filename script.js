...
Object.assign(UI_TRANSLATIONS.en,{locationLoading:'Loading local time and weather…',locationUnavailable:'Weather unavailable'});
Object.assign(UI_TRANSLATIONS.fr,{locationLoading:'Chargement de l’heure locale et de la météo…',locationUnavailable:'Météo indisponible'});
...
const closeMobileMenu=()=>{navLinks.classList.remove('open');menuButton.setAttribute('aria-expanded','false')};
...
document.addEventListener('pointerdown',event=>{if(navLinks.classList.contains('open')&&!navLinks.contains(event.target)&&!menuButton.contains(event.target))closeMobileMenu()});
...
const locationStatus=document.getElementById('portfolio-location-status');
const formatLocationTime=timezone=>{try{return new Intl.DateTimeFormat(currentLanguage==='fr'?'fr-FR':'en-GB',{hour:'2-digit',minute:'2-digit',second:'2-digit',timeZone:timezone}).format(new Date())}catch(error){return ''}};
...
function renderLocationPanel(){if(portfolioLocationState)locationStatus.textContent=locationReading(`📍 ${portfolioLocationState.name}`,portfolioLocationState)}
...
async function loadPortfolioLocation(location,force=false){if(portfolioLocationState?.source===location&&!force){renderLocationPanel();return}locationStatus.textContent=t('locationLoading');try{const response=await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(location)}&count=1&language=en&format=json`);if(!response.ok)throw new Error('Location unavailable');const place=(await response.json()).results?.[0];if(!place)throw new Error('Location unavailable');const weather=await fetchCurrentWeather(place.latitude,place.longitude);portfolioLocationState={...weather,name:[place.name,place.country].filter(Boolean).join(', '),source:location};renderLocationPanel()}catch(error){locationStatus.textContent=`📍 ${location} · ${t('locationUnavailable')}`}}
function requestCurrentLocation(){if(!navigator.geolocation||locationAccessDenied||visitorLocationRequestPending)return;visitorLocationRequestPending=true;navigator.geolocation.getCurrentPosition(async position=>{try{visitorLocationState=await fetchCurrentWeather(position.coords.latitude,position.coords.longitude)}catch(error){}finally{visitorLocationRequestPending=false}},error=>{if(error.code===error.PERMISSION_DENIED)locationAccessDenied=true;visitorLocationRequestPending=false},{enableHighAccuracy:false,timeout:10000,maximumAge:240000})}
locationClockTimer=window.setInterval(renderLocationPanel,1000);
async function loadPortfolio(){
  try{
    ...
    locationRefreshTimer=window.setInterval(()=>{loadPortfolioLocation(data.location,true);requestCurrentLocation()},300000);
